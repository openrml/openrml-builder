...
// Определяем тип для элемента payload
...
function ChartTooltipContent({
  ...
  // Приводим payload к правильному типу
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {labelFormatter(value, typedPayload as unknown as any[])}
        </div>
      );
    }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-1.5">
        {typedPayload.map((item: PayloadItem, index: number) => {
          ...
          return (
            ...
            >
              {formatter && item?.value !== undefined && item.name ? (
                formatter(item.value, item.name, item as unknown as any, index, (item.payload ?? []) as unknown as any[])
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
...
function ChartLegendContent({
  ...
  hideIcon?: boolean;
  nameKey?: string;
  payload?: any[]; // Переопределяем тип payload
